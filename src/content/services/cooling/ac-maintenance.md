---
title: "AC Maintenance"
short: "Spring tune-ups that catch problems before the first heat wave."
metaTitle: "AC Tune-Up & Maintenance Edmonton — Spring Service | Halo Air"
metaDesc: "Annual AC maintenance in Edmonton — 18-point spring tune-up, refrigerant pressure check, written condition report, warranty log filed. Call 780-224-0024 to book."
h1: "AC tune-ups — small spring visits that prevent July emergencies."
icon: "snowflake"
accent: "blue"
order: 23
heroImage: "ac-service"
features:
  - "18-point spring AC tune-up"
  - "Refrigerant pressure + superheat/subcool verification"
  - "Condenser coil cleaning included"
  - "Written condition report with photos"
  - "Warranty-preserving service log filed for every visit"
faq:
  - q: "Do I really need to service my AC every year?"
    a: "Yes — for two reasons. First, dirty condenser coils alone can drop AC efficiency 20% (your hydro bill notices). Second, most AC manufacturer warranties require documented annual service. Skip it, and a $1,500 compressor failure in year 8 isn't covered. A spring tune-up is the cheapest insurance you can buy."
  - q: "When's the best time to book?"
    a: "April or early May — before the first warm stretch loads the schedule. By mid-June we're booked 2–3 weeks out. We do AC service into July and August, but the system's already running hard by then; finding issues before the season starts is the point."
  - q: "What's included?"
    a: "18-point inspection: refrigerant pressure (high + low side), superheat/subcool calculation, condenser coil clean, evaporator coil inspection, capacitor microfarad test, contactor inspection, refrigerant line insulation check, condensate drain flush, blower static pressure, thermostat cooling-mode calibration, electrical disconnect inspection, pad and clearance check, written report with photos. Most visits take 60–75 minutes."
  - q: "How much does a tune-up cost?"
    a: "Call us for a current quote — pricing depends on the equipment and condition. We give a written estimate before any work begins. (Bundled monthly/yearly maintenance plans are coming — we'll add details once they launch.)"
  - q: "Will you top up refrigerant during a tune-up?"
    a: "Only if it's needed AND we've located the leak. Topping up without finding the leak is a bandaid — the refrigerant we add will be gone in weeks or months, and you've paid for it once with no fix. We measure pressures, calculate superheat/subcool, and inspect flare fittings; if pressure's low we recommend a leak hunt as the next step, not a top-up."
---

## Why annual AC maintenance pays for itself

A dirty AC coil drops efficiency 15–25%. A weak capacitor lets a compressor draw more current than it should — once per cycle, every time, for months — until the compressor windings fail. A blocked condensate drain floods a basement on the first humid week of July. Each of these is caught in a 60-minute spring tune-up; each costs $1,000+ to repair as an emergency call.

The other half of the case is **warranty preservation**. Most AC manufacturers require documented annual service to honour their 10-year parts warranty. If your compressor fails in year 8 and there's no service log, you're paying out of pocket.

## What our spring AC tune-up covers (18 points)

**Refrigerant**
- High-side and low-side pressure measurement
- Superheat calculation (vs. manufacturer target)
- Subcool calculation
- Refrigerant line insulation check
- Flare fitting visual + electronic leak detector spot-check

**Electrical & controls**
- Capacitor microfarad test (against rated value)
- Contactor inspection and amperage draw
- Condenser fan motor amperage
- Electrical disconnect and breaker inspection
- Thermostat cooling-mode calibration

**Coils & airflow**
- Outdoor condenser coil cleaning (water + coil-safe cleaner)
- Indoor evaporator coil visual inspection
- Blower wheel and filter inspection
- Static pressure measurement
- Supply-air / return-air temperature differential

**Condensate**
- Drain pan inspection
- Drain line flush
- Condensate pump test (if equipped)

**Written deliverable**
- Photo-documented condition report
- Recommended-action list with priority levels
- Filed service log entry (warranty preservation)

## When to book — and why spring

Best time to book an AC tune-up is **April or early May** — before the first 25°C+ stretch loads the dispatch board. By mid-June we're typically booked 2–3 weeks out. A spring service finds problems while they're still cheap to fix and gives you a clean start to the cooling season.

If you've never had your AC serviced and it's over 5 years old, this is the most important call you'll make this spring. If you bought a home in the last 18 months and don't know the service history, same answer.

## Maintenance plans — coming soon

We're building bundled monthly and yearly maintenance plans (spring AC + fall furnace tune-ups, priority dispatch, repair discounts). They aren't live yet — call for one-off tune-up pricing in the meantime, and we'll loop you in when the plans launch.

## Book an AC tune-up

Call **780-224-0024** or [book online](/book/) to get on the spring schedule. Most service appointments confirm within 2 business hours.
