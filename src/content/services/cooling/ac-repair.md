---
title: "AC Repair"
short: "Fast diagnosis and repair for residential and light-commercial cooling."
metaTitle: "AC Repair Edmonton — Same-Day Service, All Brands | Halo Air"
metaDesc: "Edmonton AC repair from a refrigerant-licensed Red Seal technician. Diagnostic visit, written quote before work, all major brands. Call 780-224-0024 or book online."
h1: "AC repair in Edmonton — diagnosed today, fixed without surprises."
icon: "snowflake"
accent: "blue"
order: 21
heroImage: "ac-service"
features:
  - "Same-day service when you call before noon"
  - "Refrigerant-licensed (R-410A and R-32 compatible)"
  - "Written quote before any work starts — no surprise line items"
  - "All major brands serviced — Lennox, Trane, Carrier, Goodman, York, Daikin, Mitsubishi"
  - "90-day repair labour warranty in writing"
faq:
  - q: "How much does AC repair cost in Edmonton?"
    a: "Most residential AC repairs fall between $150 and $650 depending on the failed component. A blown capacitor runs ~$180 installed; a contactor replacement is ~$220; a compressor replacement on an older unit can exceed $1,800, at which point we'd give you a side-by-side cost comparison against full replacement. Every Halo Air quote is written and given before we start work — no surprises."
  - q: "How fast can you respond to a no-cool call?"
    a: "If you call before noon on a weekday, we'll usually be on-site that same afternoon. For verified emergencies (medical equipment, vulnerable household members, indoor temps above 30°C), we prioritize and after-hours service is available on request — call or email and we'll work to get to you as soon as possible."
  - q: "Should I repair or replace my AC?"
    a: "Two rules of thumb: (1) if the repair quote exceeds 30% of the cost of a new unit, replacement usually wins on lifetime cost; (2) if the system is over 12 years old and uses R-22 refrigerant, replacement is almost always the better call because R-22 is phased out and refilling costs are punitive. We give you both numbers so the decision is yours."
  - q: "Do you service all AC brands?"
    a: "Yes. We service every residential split-system, mini-split, and packaged unit sold in Canada — Lennox, Trane, Carrier, Goodman, York, Daikin, Mitsubishi, LG, Bryant, American Standard, Rheem, and others. Warranty work depends on the brand and dealer agreement, but we can usually coordinate."
  - q: "What's included in a diagnostic visit?"
    a: "A diagnostic visit covers electrical testing (capacitor, contactor, control board), refrigerant pressure check, airflow assessment, condenser coil inspection, and thermostat verification. You get a written report with the root cause, a repair quote, and a recommended next step. Diagnostic fee is waived if you proceed with the repair."
---

## What we fix on AC repair calls

Edmonton summers swing from cool to 30°C+ in a few days, and that stress is what breaks air conditioners. The most common failures we see — in rough order of frequency — are blown capacitors, dirty condenser coils, refrigerant leaks at flare fittings, failed contactors, and frozen evaporator coils caused by low airflow. Each has a different fix and a different price, and we'll always show you which one yours is before we start.

### Symptom-to-cause cheat sheet

- **AC is running but the air isn't cold** — usually low refrigerant (which means there's a leak somewhere), or a dirty condenser coil. Topping up refrigerant without finding the leak just delays the next failure by a few weeks.
- **AC won't start at all** — most often a blown capacitor, sometimes a failed contactor, occasionally a tripped high-pressure switch or a control board fault.
- **AC starts then trips the breaker** — compressor electrical fault, hard-start issue, or oversized refrigerant charge.
- **AC runs constantly but the house isn't cooling** — undersized system, blocked return-air path, leaking ductwork, or a thermostat calibration issue.
- **Ice on the indoor coil or refrigerant line** — almost always low airflow (dirty filter, blocked vents, blower issue) or low refrigerant.

## How a Halo Air AC repair visit works

1. **Booking.** You call 780-224-0024 or [book online](/book/). We confirm your appointment with a 2-hour window.
2. **Diagnosis.** Our technician walks the system end-to-end: thermostat → indoor coil → outdoor condenser → electrical → refrigerant. Most diagnoses take 30–45 minutes.
3. **Quote.** You get a written repair quote on the spot. If multiple fixes are viable (repair vs. replace), we present both with real numbers — not pressure.
4. **Repair.** We carry common parts on the truck (capacitors, contactors, fuses, thermostats) so a majority of repairs are done in the same visit. Heavier parts (compressors, coils, blower motors) are ordered with a confirmed install date.
5. **Sign-off.** Test under load, clean up, document the repair, leave the parts box behind so you can see what was replaced. Warranty paperwork is emailed.

## When repair stops making sense

If your system is older than 12 years, uses R-22 refrigerant, has had two or more major repairs in the last 24 months, or is undersized for the home — we'll tell you honestly that replacement is the better lifetime decision. We can quote a [new AC install](/services/cooling/ac-installation/) the same week, often paired with a [furnace replacement](/services/heating/furnace-installation/) for combined-system pricing.

## What Halo Air customers get that big-box HVAC doesn't

- **Live local dispatch.** The number on this page rings a real person in Edmonton, not an offshore call centre.
- **Written upfront pricing.** No "we'll send you the bill" — you see the number before we lift a tool.
- **Three trades under one roof.** If the diagnosis turns out to involve plumbing, gas, or refrigeration, we don't sub it out — we're licensed for all three.
- **Warranty backed in writing.** Every repair is documented and warranted. Keep the email; we honour the dates without asking for the original work order.

## Book your AC repair today

Edmonton summers don't wait. If your AC is acting up, the cheapest fix is usually the one you address before the heat wave. Call **780-224-0024** for same-day service or [request a visit online](/book/) — most online requests get a callback within 2 business hours.
